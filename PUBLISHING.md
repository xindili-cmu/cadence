...
```bash
node scripts/linkedin-brief.js      # → linkedin/日期/ post-en/zh.txt + sources-en/zh.txt
node scripts/linkedin-daily-card.js # → linkedin/日期/daily-signal.png（1080×1350 信号卡）
```
...
5. 发完把 `sources-en.txt` 粘到第一条评论。

> 话题标签 `#xxx` 以纯文本保留（勿触发 LinkedIn 话题选择器）。Chrome 未连 / 未登录 → 优雅回退到 chat 粘贴，不硬失败。
> 自动化：scheduled task `linkedin-daily-post`（cron `0 14 * * 1-5`，Mon–Fri 2pm PT），产文本+卡片后驱动 Chrome 到 pre-publish 停在 Post。
...
1. Resend → Domains → 验证 `incadencept.com`（DNS 记录加在 Cloudflare）。
2. Resend → Audiences/Segments → 建 `weekly-signal` → 复制 ID。
3. GitHub repo → Variables：`RESEND_SEGMENT_ID`、`MAIL_FROM = Cadence 步频 <weekly@incadencept.com>`（Secret `RESEND_API_KEY` 已有）。
4. 首期发送成功后，把网站订阅卡文案从「筹备中」改回承诺式（`sub.body`，届时「一键退订」由 Resend 兜底，承诺才为真）。

**退订**：邮件尾部 `{{{RESEND_UNSUBSCRIBE_URL}}}` 由 Resend 托管，一键退订自动从 segment 移除，无需人工处理。
